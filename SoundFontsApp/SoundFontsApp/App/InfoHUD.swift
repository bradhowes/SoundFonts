...
final class InfoHUD: NSObject {
  ...
  static private var rootView: UIView? {
  ...
  static func show(text: String, duration: TimeInterval = 3.0) {
    ...
    let superFrame = CGRect(
      x: 0, y: 0, width: label.frame.width + 50, height: label.frame.height + 30)
...
extension Selector {
  fileprivate static let hide = #selector(InfoHUD.hide(_:))
}
