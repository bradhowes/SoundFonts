...
class PresetChangeManagerTests: XCTestCase {
  ...
  func testLoading() throws {
    ...
    presetChangeManager.change(sampler: synth, url: urls[0], preset: Preset("", 0, 0, 0)) { result in
      switch result {
      case .success: self.loadFinishedExpectation?.fulfill()
      case .failure: XCTFail("unexpected failure")
      }
    }
    presetChangeManager.change(sampler: synth, url: urls[0], preset: Preset("", 0, 0, 1)) { result in
      switch result {
      case .success: self.loadFinishedExpectation?.fulfill()
      case .failure: XCTFail("unexpected failure")
      }
    }
    presetChangeManager.change(sampler: synth, url: urls[0], preset: Preset("", 0, 0, 2)) { result in
      switch result {
      case .success: self.loadFinishedExpectation?.fulfill()
      case .failure: XCTFail("unexpected failure")
      }
  ...
  func testFailures() throws {
    ...
    presetChangeManager.change(sampler: synth, url: urls[3], preset: Preset("", 0, 0, 1)) { result in
      switch result {
