...
extension XCTestCase {
  ...
  public typealias TestBlock = (CoreDataTestHarness, NSManagedObjectContext) -> Void

  public func doWhenCoreDataReady(
    _ name: String, createAppState: Bool = true, block: @escaping TestBlock
  ) {
...
extension CoreDataTestHarness {
  ...
  public func fetchEntities<T>() -> [T]? where T: Managed {
...
public struct CoreDataTestData {
  ...
  static func make(_ name: String) -> SoundFontInfo {
    SoundFontInfo(
      name, url: URL(fileURLWithPath: "SF3.sf2"),
      author: "Author",
      comment: "Comment",
      copyright: "Copyright",
      presets: [
