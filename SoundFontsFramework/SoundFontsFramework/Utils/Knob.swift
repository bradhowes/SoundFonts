...
open class Knob: UIControl {
  ...
  open var value: Float {
    get { _value }
    set { setValue(clampedValue(newValue), animated: false) }
  }
  ...
  open var indicatorColor: UIColor = .systemTeal {
  ...
  /// The color of the radial line drawn from the current value on the arc towards the arc center.
  open var radialLineColor: UIColor = .systemTeal {
    didSet { radialLayer.strokeColor = radialLineColor.cgColor }
  }
  ...
  open var tickLineColor: UIColor = .systemTeal {
    didSet { ticksLayer.strokeColor = tickLineColor.cgColor }
  }
...
extension Knob {
  ...
  override open func continueTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
    ...
    let deltaT =
      Float(panOrigin.y - point.y)
      / (Float(min(bounds.height, bounds.width)) * touchSensitivity * scaleT)
...
extension Knob {
  ...
  private func createTrack() {
    let ring = UIBezierPath(
      arcCenter: CGPoint.zero, radius: radius, startAngle: CGFloat(startAngle),
  ...
  private var radius: CGFloat {
    (min(trackLayer.bounds.width, trackLayer.bounds.height) / 2) - trackLineWidth
  }

  private var angleForValue: Float {
    angle(for: (_value - minimumValue) / (maximumValue - minimumValue))
  }

  private func angle(for normalizedValue: Float) -> Float {
    normalizedValue * (endAngle - startAngle) + startAngle
  }
