...
extension AudioUnitParameters {
    ...
    var state: [String: Float] {
        ...
        return [self.enabled.identifier: enabled, self.time.identifier: time, self.feedback.identifier: feedback, self.cutoff.identifier: cutoff,
                self.wetDryMix.identifier: wetDryMix]
    }
    ...
    func setState(_ state: [String: Any]) {
        ...
        guard let time = state[self.time.identifier] as? Float else {
            os_log(.error, log: log, "missing '%s' in state", self.cutoff.identifier)
            return
        }
        self.time.value = time

        guard let feedback = state[self.feedback.identifier] as? Float else {
            os_log(.error, log: log, "missing '%s' in state", self.feedback.identifier)
            return
        }
        self.feedback.value = feedback
